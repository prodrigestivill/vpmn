...
\end{figure}
Per tant podem dir que Wippien crea \okeyword{VPN}s mallades i completament connectades per cada anella de confiança, amb autenticació (i assignació de les IPs) per part d'un servidor central extern (veure la figura \ref{F:vpn-externalized}).
...
A nivell de seguretat utilitza un protocol propi amb AES128, mitjançant un component privatiu anomenat \emph{wodVPN ActiveX}, aquest component és \emph{NAT friendly} (està preparat per travessar els routers \okeyword{NAT}) i suporta tant TCP com UDP.
