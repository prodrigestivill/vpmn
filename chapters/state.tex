...
\end{figure}
Per tant podem dir que Wippien crea \okeyword{VPN}s mallades i completament connectades per cada anella de confiança, amb autenticació (i assignació de les IPs) per part de un servidor central extern (veure la figura \ref{F:vpn-externalized}).

Utilitza una única interfície de xarxa en cada màquina per tota la VPN, com en la topologia interna \emph{subnet} de OpenVPN. Per tant, és cada \emph{peer} qui s'encarrega de l'enrutat de la VPN a nivell d'aplicació. Totes les IPs de les \okeyword{VPN}s que utilitzin el seu servidor \emph{mediator} pertanyen a la xarxa 5.0.0.0/8.

A nivell de seguretat utilitza un protocol propi amb AES128, mitjançant un component privatiu anomenat \emph{wodVPN ActiveX}, aquest component es \emph{NAT friendly} (està preparat per travessar els routers \okeyword{NAT}) i suporta tant TCP com UDP.

\subsection{Hamachi}
Hamachi és un programa privatiu de \okeyword{VPN}s a nivell d'aplicació que suporta tant túnels IP com túnels \keyword{IPX}{Internetwork Packet Exchange}.
El programa utilitza una connexió a un servidor central com a canal de control, en arrancar efectua el procés d'autenticació amb aquest servidor i s'efectua el descobriment del canal del client per assegurar la connectivitat en cas d'existir routers NAT. Finalment el servidor entrega al client la llista de \okeyword{VPN}s del client juntament amb la llista dels seus membres que s'actualitza constantment cada cop que aquests entren o surten de l'aplicació. Tal com s'ha dit el servidor ajuda en el proces dels clients de superar els routers NAT (és una aplicació \emph{NAT friendly}), en el procés d'autenticació i en el descobriment de \emph{peers} (veure la figura \ref{F:vpn-externalized}).

Utilitza una única interfície de xarxa en cada màquina per tota la \okeyword{VPN}. Per tant, és cada \emph{peer} qui s'encarrega de l'enrutat de la \okeyword{VPN} a nivell d'aplicació. Totes les IPs de les \okeyword{VPN}s pertanyen a la xarxa 5.0.0.0/8.
