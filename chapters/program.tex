...
Els altres paquets, considerats d'ús exclusivament intern, fan la funció de senyalització de la VPN. Aquests tenen un format que està inspirat en els 4 primers bytes de la capçalera IP, es poden diferenciar per el primer byte que en IP defineix la versió del protocol i la longitud de les capçaleres (HLen). El camp equivalent al que en IP està destinat a \keyword{ToS}{Type of Service} en els paquets de senyalització servirà per definir el tipus de paquet d'intern (veure taula \ref{T:inpkt}).
\begin{table}[hbt]
\begin{center}
\scriptsize
...
\end{center}
\end{table}
Els tipus de paquets són:
\begin{itemize}
\item \textbf{Paquets \emph{Identification}}: Descriuen al node que l'envia.
\item \textbf{Paquet \emph{Identification Acknowledgment}}: Confirmen haver rebut el paquet \emph{Identification}.
\item \textbf{Paquet \emph{Keep Alive}}: Mantenen el canal obert.
\end{itemize}
En la figura \ref{F:dia-pktflow} es pot veure un diagrama que mostra el seu ús dins de la comunicació.
\begin{figure}[htb]
\centering
...
Els paquets \emph{Identification} (ID) descriuen al node que l'envia. Per descriure'l s'envia la següent informació:
\begin{itemize}
\item Xarxes que publica el node, també fa d'identificador del node.
\item Agrupacions de IP i port, altres vies per comunicar-se amb el node.
\end{itemize}
En el paquet (taula \ref{T:pktid}) primer s'envien les quantitats de la informació que s'envia, tot seguit s'envien totes les xarxes que el node publica i desprès les agrupacions de IP i port.
\begin{table}[htb]
\begin{center}
\scriptsize
\begin{tabular}{|c|p{0.0625\linewidth}|p{0.0625\linewidth}|p{0.125\linewidth}|p{0.25\linewidth}c|}
...
El paquet \emph{Identification Acknowledgment} (ID ACK) confirma haver rebut el paquet ID (taula \ref{T:pktidack}).
\begin{table}[htb]
\begin{center}
\scriptsize
\begin{tabular}{|c|p{0.0625\linewidth}|p{0.0625\linewidth}|p{0.125\linewidth}|p{0.25\linewidth}c|}
...
El paquet \emph{Keep Alive} (KA) manté el canal obert i envia informació d'altres nodes (veure taula \ref{T:pktka}).
%Hole punching.
