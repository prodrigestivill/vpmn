...
\end{center}
\end{table}
Per a aquesta aplicació només es farà ús dels permisos de tipus IP. D'aquesta manera és la \okeyword{CA} l'encarregada d'assegurar-se que no validi cap certificat amb una IP ja assignada prèviament, així com també de mantenir un registre de les IPs assignades.
...
\end{center}
\end{table}
La resta de paquets considerats d'ús exclusivament intern, fan la funció de senyalització de la VPN. Inspirats amb els paquets IP es podran diferenciar per el primer byte que en IP defineix la versió del protocol i la longitud de les capçaleres (HLen). La part de IP destinada a \keyword{ToS}{Type of Service} servirà per definir el tipus de paquet (veure taula \ref{T:inpkt}).
\begin{table}[hbt]
\begin{center}
\scriptsize
